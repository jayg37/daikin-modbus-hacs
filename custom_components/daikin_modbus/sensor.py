...
async def async_setup_entry(hass: HomeAssistant, entry: DaikinConfigEntry, async_add_entities: AddConfigEntryEntitiesCallback) -> None:
    """Set up read-only register sensors."""
    async_add_entities(DaikinRegisterSensor(entry.runtime_data, field, name, unit) for field, name, unit in SENSORS)
